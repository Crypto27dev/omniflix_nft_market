...
const PriceRange = (props) => {
        ...
        if (newValue[1] - newValue[0] < minDistance) {
            if (activeThumb === 0) {
                ...
                if (props.value) {
                    ...
                    props.fetchListedCollectionNFTs(route, props.listedNFTsSkip, props.listedNFTsLimit, [props.value.ibc_denom_hash || props.value.value], minValue, maxValue, props.sortBy, props.order, props.listedSearch);
                }
            } else {
    ...
    const handleChange = (e) => {
        ...
        if (props.range && props.range.length) {
            ...
            props.fetchListedCollectionNFTs(route, props.listedNFTsSkip, props.listedNFTsLimit, [props.value.ibc_denom_hash || props.value.value], null, null, props.sortBy, props.order, props.listedSearch);
        }
...
const stateToProps = (state) => {
    return {
        ...
        lang: state.language,
        list: state.collection.filters.priceRangeCollection.list,
        max: state.collection.filters.priceRangeCollection.max,
        min: state.collection.filters.priceRangeCollection.min,
        onSaleIn: state.collection.filters.onSaleInCollection.value,
        range: state.collection.filters.priceRangeCollection.range,
        value: state.collection.filters.priceRangeCollection.value,
...
const actionToProps = {
    ...
    onChange: setCollectionPriceRangeCoin,
